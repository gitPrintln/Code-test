...
public class 호텔방배정 {
     ...
     */
    class Solution {
        private Map<Long, Long> nextRoom = new HashMap<>(); // 방 번호별 다음 빈 방을 기록
        ...
        private long emptyRoom(long room) {
            // 방이 비어있으면 바로 배정하고, 다음 빈 방을 room + 1로 설정
            if (!nextRoom.containsKey(room)) {
                nextRoom.put(room, room + 1);
                return room;
            }
            
            // 이미 배정된 경우 재귀적으로 다음 빈 방을 찾아 업데이트
            long nextAvailable = emptyRoom(nextRoom.get(room));
            nextRoom.put(room, nextAvailable); // 경로 압축
