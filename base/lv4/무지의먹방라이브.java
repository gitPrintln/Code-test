    ...
    * 효율성 테스트에 부분 점수가 있는 문제입니다.
    ...
    class Solution {
        public int solution(int[] food_times, long k) {
            ...
            // 최소 시간 음식부터 먹어치우며, 남은 시간을 줄여 나갑니다.
            long previousTime = 0;
            for (int i = 0;i < n;i++) {
                // 남은 음식 개수와 걸리는 시간 계산
                long currentTime = foodList.get(i)[0];
                long timeDiff = currentTime - previousTime;
                long timeToSpend = timeDiff * (n - i);
                
                // k 초보다 시간이 더 필요하다면 중단하고 나머지를 계산
                if (timeToSpend > k) {
                    // 남은 음식들 중 몇 번째 음식인지 계산
                    foodList = foodList.subList(i, n);
                    foodList.sort(Comparator.comparingInt(a -> a[1]));  // 원래 순서대로 정렬
                    return foodList.get((int)(k % foodList.size()))[1];
                }
