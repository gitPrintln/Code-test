...
public class 퍼즐게임챌린지 {
    ...
    class Solution {
        public int solution(int[] diffs, int[] times, long limit) {
            ...
            while (low < high) {
                ...
                if (canSolveWithinLimit(diffs, times, mid, limit)) {
                    high = mid;  // 더 작은 숙련도를 탐색
                } else {
                    low = mid + 1;  // 더 큰 숙련도를 탐색
                }
            }
        ...
        // 주어진 숙련도로 제한 시간 내에 퍼즐을 모두 풀 수 있는지 확인하는 함수
        private boolean canSolveWithinLimit(int[] diffs, int[] times, int level, long limit) {
            long totalTime = 0;
            int n = diffs.length;

            // 첫 번째 퍼즐은 무조건 틀리지 않으므로 바로 해결 가능
            totalTime += times[0];
            ...
            for (int i = 1;i < n;i++) {
                if (diffs[i] <= level) {
                    ...
                    // 숙련도가 부족하면 diff - level번 틀리고, 그 후 다시 푸는 경우
                    int mistakes = diffs[i] - level;
                    totalTime += mistakes * (times[i] + times[i - 1]) + times[i];
                }
