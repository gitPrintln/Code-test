...
public class 스타수열 {
    ...
    class Solution {
        public int solution(int[] a) {
            ...
            // 모든 숫자에 대해 스타 수열을 구성할 수 있는지 확인합니다.
            for (int key : frequency.keySet()) {
                // 이 숫자의 등장 횟수가 적다면, 이 숫자를 스타 수열로 구성할 수 없습니다.
                if (frequency.get(key) <= maxLength / 2) continue;

                int length = 0;

                // 배열을 순차적으로 순회하면서 key와 다른 숫자가 있는 쌍을 만듭니다.
                for (int i = 0;i < a.length - 1;i++) {
                    if ((a[i] == key || a[i + 1] == key) && a[i] != a[i + 1]) {
                        length += 2;
                        i++; // 다음 짝을 위해 한 칸 건너뜁니다.
                    }
                }
