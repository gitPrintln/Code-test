...
public class 양과늑대2 {
    ...
    class Solution {
        ...
        private static void bfs() {
            ...
            startList.add(0); // 시작 노드는 항상 0번

            // 초기 상태: 위치 0번, 양 0마리, 늑대 0마리, 시작 리스트
            queue.add(new State(0, 0, 0, startList));

            while (!queue.isEmpty()) {
                State current = queue.poll();
                int idx = current.idx;
                int sheepCnt = current.sheepCnt;
                int wolfCnt = current.wolfCnt;
                List<Integer> nextPos = current.nextPos;

                // 현재 노드 처리
                if (Info[idx] == 0) sheepCnt++; // 양일 경우
                else wolfCnt++; // 늑대일 경우

                // 늑대가 양의 수를 넘으면 탐색 중단
                if (wolfCnt >= sheepCnt) continue;

                // 최대 양의 수 갱신
                maxSheepCnt = Math.max(maxSheepCnt, sheepCnt);

                // 다음 탐색할 노드 갱신
                List<Integer> newNextPos = new ArrayList<>(nextPos);
                newNextPos.remove(Integer.valueOf(idx)); // 현재 노드 제거
                if (childs[idx] != null) {
                    for (int child : childs[idx]) {
                        newNextPos.add(child); // 자식 노드 추가
                    }
                }

                // 다음 탐색 가능한 모든 노드를 큐에 추가
                for (int next : newNextPos) {
                    queue.add(new State(next, sheepCnt, wolfCnt, newNextPos));
                }
            }
        }
        ...
        // 상태를 저장하는 클래스
        static class State {
            int idx, sheepCnt, wolfCnt;
            List<Integer> nextPos;

            State(int idx, int sheepCnt, int wolfCnt, List<Integer> nextPos) {
                this.idx = idx;
                this.sheepCnt = sheepCnt;
                this.wolfCnt = wolfCnt;
                this.nextPos = new ArrayList<>(nextPos);
            }
        }
