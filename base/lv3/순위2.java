    ...
    class Solution {
        public int solution(int n, int[][] results) {
            ...
            for (int[] result : results) {
                winGraph.get(result[0]).add(result[1]); // 이긴 선수 추가
                loseGraph.get(result[1]).add(result[0]); // 진 선수 추가
            }
            ...
            for (int i = 1;i <= n;i++) {
                ...
                int winCount = dfs(winGraph, visitedWin, i);
                int loseCount = dfs(loseGraph, visitedLose, i);
