...
public class 불량사용자2 {
    ...
    class Solution {
        public int solution(String[] user_id, String[] banned_id) {
            // 불량 사용자 아이디에 매칭되는 사용자 아이디를 찾아서 저장할 리스트
            List<List<String>> possibleMatches = new ArrayList<>();
            
            // 각 banned_id에 대해 매칭 가능한 user_id들을 찾는다.
            for (String banned : banned_id) {
                ...
                possibleMatches.add(candidates);
            }

            // 중복을 제거할 Set
            Set<Set<String>> resultSet = new HashSet<>();

            // 모든 가능한 제재 아이디 조합을 구한다.
            findCombinations(possibleMatches, new HashSet<>(), 0, resultSet);

            return resultSet.size();
        }

        // 가능한 제재 아이디 조합을 구하는 함수
        private void findCombinations(List<List<String>> possibleMatches, Set<String> currentSet, int idx, Set<Set<String>> resultSet) {
            if (idx == possibleMatches.size()) {
                // 모든 banned_id에 대해 선택된 user_id들을 Set으로 저장
                resultSet.add(new HashSet<>(currentSet));
                return;
            }

            for (String user : possibleMatches.get(idx)) {
                if (!currentSet.contains(user)) {
                    currentSet.add(user);
                    findCombinations(possibleMatches, currentSet, idx + 1, resultSet);
                    currentSet.remove(user);
                }
            }
        }

        // user_id와 banned_id가 일치하는지 확인하는 함수
