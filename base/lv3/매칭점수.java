...
public class 매칭점수 {
    ...
    class Solution {
        public int solution(String word, String[] pages) {
            ...
            // 각 페이지의 기본 점수와 외부 링크 수, 링크 점수 저장 배열
            double[] baseScores = new double[n];
            double[] linkScores = new double[n];
            List<List<String>> externalLinks = new ArrayList<>();
            
            // 정규 표현식을 사용해 URL, 외부 링크, 검색어 매칭을 찾음
            ...
            // 링크 점수 계산
            for (int i = 0;i < n;i++) {
                List<String> links = externalLinks.get(i);
                if (links.size() > 0) {
                    double linkValue = baseScores[i] / links.size();
                    for (String link : links) {
                        Integer linkedPageIndex = pageIndexMap.get(link);
                        if (linkedPageIndex != null) {
                            linkScores[linkedPageIndex] += linkValue;
                        }
                    }
                }
            }
            ...
            int answer = 0;
            for (int i = 0;i < n;i++) {
                double matchScore = baseScores[i] + linkScores[i];
