...
public class 비밀코드해독 {
    ...
    class Solution {
        ...
        public int solution(int n, int[][] q, int[] ans) {
            ...
            generate(1, n, 5, combination, q, ans);
            return result;
        }

        // 5개 조합을 백트래킹으로 생성
        void generate(int start, int n, int k, List<Integer> comb, int[][] q, int[] ans) {
            if (comb.size() == k) {
            ...
            for (int i = start;i <= n;i++) {
                comb.add(i);
                generate(i + 1, n, k, comb, q, ans);
                comb.remove(comb.size() - 1);
            }
        }

        boolean isValid(List<Integer> code, int[][] q, int[] ans) {
