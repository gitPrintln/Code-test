...
public class 이웃한칸 {
    ...
    class Solution {
        public int solution(String[][] board, int h, int w) {
            ...
            int count = 0; // 같은 색깔 개수
            int[] dh = {0, 1, -1, 0}; // 행 변화량 (우, 하, 상, 좌)
            int[] dw = {1, 0, 0, -1}; // 열 변화량

            String target = board[h][w]; // 기준 색깔
            ...
            for (int i = 0;i < 4;i++) {
                ...
                if (h_check >= 0 && h_check < n && w_check >= 0 && w_check < n) {
                    if (board[h_check][w_check].equals(target)) { // 인접한 칸의 색이 기준 색(targetColor)과 같으면
                        count++;
                    }
                }
            }
